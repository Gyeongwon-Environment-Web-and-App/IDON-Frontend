...
const MapSideMenu: React.FC<MapSideMenuProps> = ({ onSidebarChange }) => {
  ...
  const { activeSidebar, setActiveSidebar, sidebarOpen } =
    useMapOverviewStore();
  // !const { activeSidebar, selectedComplaintId, setActiveSidebar, sidebarOpen } = useMapOverviewStore();
  const selectedComplaintId = "1";
    ...
    <div className="w-screen h-screen">
      ...
      {activeSidebar && (
        ...
        >
          ...
          <div className={`flex w-full text-[0.73rem] md:text-sm border border-light-border rounded ${activeSidebar === "complaint" ? 'hidden' : 'block'}`}>
