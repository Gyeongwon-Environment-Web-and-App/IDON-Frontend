...
import type { StatisticsData, TransformedStatisticsData } from '@/types/statistics';
import {
...
export const useStatistics = () => {
  ...
  const [transformedData, setTransformedData] = useState<TransformedStatisticsData | null>(null);
    ...
    async (categories:string[], dateRange?: DateRange) => {
      ...
      try {
        ...
        const transformed: TransformedStatisticsData = {
          positiveNegatives: transformPosNegToChartData(data.positiveNegatives, categories[0]),
          days: transformDaysToBarChartData(data.days, categories[0]),
          timePeriods: transformTimePeriodsToBarChartData(data.timePeriods, categories[0]),
